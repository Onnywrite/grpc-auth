...
func (pg *Pg) SaveUser(ctx context.Context, user *models.User) (*models.SavedUser, ero.Error) {
	...
	stmt, err := pg.db.PreparexContext(ctx, `
		...
		VALUES ($1, $2, $3, $4)
		RETURNING user_id, nickname, email, phone, password, deleted_at`)
	if err != nil {
		return nil, preparingError(err, op)
	}

	row := stmt.QueryRowxContext(ctx, user.Nickname, user.Email, user.Phone, user.Password)
...
func (pg *Pg) UserByNickname(ctx context.Context, nickname string) (*models.SavedUser, ero.Error) {
	return pg.whereUser(ctx, "nickname = $1", nickname)
}
...
func (pg *Pg) whereUser(ctx context.Context, where string, args ...any) (*models.SavedUser, ero.Error) {
	...
	stmt, err := pg.db.PreparexContext(ctx, fmt.Sprintf(`
		SELECT user_id, nickname, email, phone, password, deleted_at
		FROM users
		WHERE %s`, where))
	if err != nil {
		return nil, preparingError(err, op)
